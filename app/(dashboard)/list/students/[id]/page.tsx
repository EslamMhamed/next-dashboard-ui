import Announcements from "@/components/Announcements";
import BigCalendar from "@/components/BigCalendar";
import Performance from "@/components/Performance";
import Image from "next/image";
import Link from "next/link";
import React from "react";

function SingleStudentPage() {
  return (
    <div className="flex flex-col xl:flex-row flex-1 p-4 gap-4">
      {/* LEFT */}
      <div className="w-full xl:w-2/3">
        {/* TOP */}
        <div className="flex flex-col lg:flex-row gap-4">
          {/* USER INFO CARD */}
          <div className="bg-lamaSky py-6 rounded-md flex gap-4 flex-1">
            <div className="w-1/3 ">
              <Image
                src="https://images.pexels.com/photos/5414817/pexels-photo-5414817.jpeg?auto=compress&cs=tinysrgb&w=1200"
                alt="pic"
                width={144}
                height={144}
                className="w-36 h-36 rounded-full object-cover"
              />
            </div>
            <div className="w-2/3 flex flex-col justify-between gap-4">
              <h1 className="text-xl font-semibold">ِLara Sayed</h1>
              <p className="text-sm text-gray-500">Lorem ipsum dolor, sit amet consectetur adipisicing elit.</p>
              <div className="flex justify-between items-center  gap-2 flex-wrap text-xs font-medium ">
                <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2 ">
                  <Image src='/blood.png' width={14} height={14} alt="blood" />
                  <span>A+</span>
                </div>
                <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2 ">
                  <Image src='/date.png' width={14} height={14} alt="date" />
                  <span>May 2025</span>
                </div>
                <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2 ">
                  <Image src='/mail.png' width={14} height={14} alt="mail" />
                  <span>lara22@gamil.com</span>
                </div>
                <div className="w-full md:w-1/3 lg:w-full 2xl:w-1/3 flex items-center gap-2 ">
                  <Image src='/phone.png' width={14} height={14} alt="phone" />
                  <span className="text-xs">+20 100550354</span>
                </div>
              </div>
            </div>
          </div>
          {/* SMALL CARDS */}
          <div className="flex-1 flex justify-between gap-4 flex-wrap">
            {/* CARD */}
            <div className="bg-white w-full p-4 rounded-md flex gap-4 md:w-[48%] xl:w-[45%] 2xl:w-[48%] ">
              <Image src='/singleAttendance.png' width={24} height={24} alt="signle" className="w-6 h-6" />
              <div className="">
                <h1 className="text-xl font-semibold ">90%</h1>
                <span className="text-gray-400 text-sm ">Attendance</span>
              </div>
            </div>
            {/* CARD */}
            <div className="bg-white w-full p-4 rounded-md flex gap-4 md:w-[48%] xl:w-[45%] 2xl:w-[48%] ">
              <Image src='/singleBranch.png' width={24} height={24} alt="signle" className="w-6 h-6" />
              <div className="">
                <h1 className="text-xl font-semibold ">6th</h1>
                <span className="text-gray-400 text-sm ">Grade</span>
              </div>
            </div>
            {/* CARD */}
            <div className="bg-white w-full p-4 rounded-md flex gap-4 md:w-[48%] xl:w-[45%] 2xl:w-[48%] ">
              <Image src='/singleLesson.png' width={24} height={24} alt="signle" className="w-6 h-6" />
              <div className="">
                <h1 className="text-xl font-semibold ">18</h1>
                <span className="text-gray-400 text-sm ">Lessons</span>
              </div>
            </div>
            {/* CARD */}
            <div className="bg-white w-full p-4 rounded-md flex gap-4 md:w-[48%] xl:w-[45%] 2xl:w-[48%] ">
              <Image src='/singleClass.png' width={24} height={24} alt="signle" className="w-6 h-6" />
              <div className="">
                <h1 className="text-xl font-semibold ">6</h1>
                <span className="text-gray-400 text-sm ">Class</span>
              </div>
            </div>
          </div>
        </div>
        {/* BOTTOM */}
        <div className="mt-4 bg-white rounded-md p-4 h-[800px]">
          <h1>Student&apos;s Schedule</h1>
          <BigCalendar />
        </div>
      </div>
      {/* RIGHT */}
      <div className="w-full xl:w-1/3 flex flex-col gap-4">
      <div className="bg-white p-4 rounded-md">
        <h1 className="text-xl font-semibold">Shortcuts</h1>
        <div className="mt-4 flex gap-4 flex-wrap text-xs text-gray-500">
          <Link href='/' className="p-3 rounded-md bg-lamaSkyLight" >Student&apos;s Lessons</Link>
          <Link href='/' className="p-3 rounded-md bg-lamaPurpleLight" >Student&apos;s Teachers</Link>
          <Link href='/' className="p-3 rounded-md bg-pink-50" >Student&apos;s Exams</Link>
          <Link href='/' className="p-3 rounded-md bg-lamaSkyLight" >Student&apos;s Assignments</Link>
          <Link href='/' className="p-3 rounded-md bg-lamaYellowLight" >Student&apos;s Results</Link>
        </div>
      </div>
      <Performance />
      <Announcements />
      </div>
    </div>
  );
}

export default SingleStudentPage;
